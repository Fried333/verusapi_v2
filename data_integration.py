...
def extract_all_pairs_data(session_id: Optional[str] = None) -> Dict:
    ...
    try:
        ...
        # Export comprehensive volume discovery data
        try:
            volume_discovery = {
                'metadata': {
                    'timestamp': datetime.utcnow().isoformat(),
                    'api_version': 'v8_multichain',
                    'chain_coverage': 'multi_chain',
                    'global_block_height': current_block
                },
                'converters': []
            }
            
            # Process each converter to extract comprehensive volume data
            for converter_idx, converter in enumerate(converters, 1):
                converter_name = converter.get('name', 'Unknown')
                currencies = get_converter_currencies(converter)
                currency_symbols = [curr['symbol'] for curr in currencies]
                ...
                converter_volume_data = {
                    ...
                    'currency_volume_calls': {},
                    'trading_pairs': []
                }
                
                # Make volume calls for each currency in this converter
                all_volume_data = {}
                for currency_info in currencies:
                    currency_symbol = currency_info['symbol']
                    
                    volume_pairs, total_volume = get_currency_volume_info(
                        converter_name, start_block, end_block, blocks_per_day, currency_symbol
                    )
                    ...
                    if volume_pairs is not None:
                        ...
                        converter_volume_data['currency_volume_calls'][currency_symbol] = {
                            'total_volume': total_volume,
                            'pairs_count': len(volume_pairs) if volume_pairs else 0,
                            'raw_pairs': volume_pairs
                        }
                
                # Extract ALL possible trading pairs (not just specific ones)
                for base_currency in currency_symbols:
                    for target_currency in currency_symbols:
                        if base_currency != target_currency:
                            # Get raw volumes from both directions
                            raw_base_volume = 0
                            raw_target_volume = 0
                            ...
                            # Include all pairs, even with zero volume for completeness
                            pair_data = {
                                'base_currency': base_currency,
                                'target_currency': target_currency,
                                'raw_base_volume': raw_base_volume,
                                'raw_target_volume': raw_target_volume,
                                'has_volume': raw_base_volume > 0 or raw_target_volume > 0,
                                'volume_sources': {
                                    'base': f"{base_currency}_call",
                                    'target': f"{target_currency}_call"
                                }
                            }
                            converter_volume_data['trading_pairs'].append(pair_data)
                
                # Only include converters that have volume data
                if converter_volume_data['currency_volume_calls']:
                    volume_discovery['converters'].append(converter_volume_data)
            
            # Add summary statistics
            chains_covered = list(set(conv['chain'] for conv in volume_discovery['converters']))
            volume_discovery['summary'] = {
                'total_converters': len(volume_discovery['converters']),
                'total_currencies': len(set(curr for conv in volume_discovery['converters'] for curr in conv['currencies'])),
                'total_pairs_with_volume': sum(
                    len([pair for pair in conv['trading_pairs'] if pair['has_volume']])
                    for conv in volume_discovery['converters']
                ),
                'chains_covered': chains_covered,
                'converters_per_chain': {chain: len([conv for conv in volume_discovery['converters'] if conv['chain'] == chain]) for chain in chains_covered}
            }
            
            volume_discovery_file = '/home/dev/Desktop/F0rked/API/API_v2/API_v8/volume_discovery.json'
            with open(volume_discovery_file, 'w') as f:
                json.dump(volume_discovery, f, indent=2)
            logger.info(f"Volume discovery data exported to {volume_discovery_file}")
        except Exception as e:
            logger.warning(f"Failed to export volume discovery data to JSON: {e}")
